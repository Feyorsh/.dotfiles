		  ...
		  `(,(make-mu4e-context
			  ...
			  :vars '((user-full-name . "George Huebner")
					  ...
					  (mu4e-bookmarks . (
										 ...
										 (:name "memo-list" :query "list:memo-list.redhat.com" :key ?m)
										 ))
					  ))))
...
mu4e-view-fields '(:from :to :cc :subject :flags :date :tags))
	...
	(setq mu4e-headers-visible-flags '(attach encrypted signed)
		  mu4e-split-view 'vertical
		  mu4e-headers-visible-columns (+ 15 10 12 22)
	...
	(define-advice mu4e~headers-human-date (:filter-return (ret) pretty-date)
	  (if (equal (format-time-string "%Y") (substring ret -4 nil))
		  (let*
