...
(use-package evil
	...
	(evil-define-command fysh/evil-save-modified-and-close (file &optional bang)
...
#+end_src
Authentication config (old, using authinfo)
#+begin_src elisp :tangle no
...
(use-package org
  ...
  :custom
  (org-agenda-files '("~/Personal/org/agenda.org")))
...
#+end_src
Adding links to PDFs with Org. Need to double check linking a PDF with eww links to the remote copy properly.
#+begin_src elisp
...
(use-package org
  ...
  (setq org-capture-templates
		'(("t" "TODO" entry (file+headline "~/Personal/org/agenda.org" "Today")
		   "* TODO %?\nDEADLINE: %t\n  %i" :empty-lines 1)
		("f" "TODO in file" entry (file+headline "~/Personal/org/agenda.org" "Today")
		   "* TODO %A\nDEADLINE: %t\n  %i" :empty-lines 1)))
  :general
...
*** Assembly
I use GAS for class, and otherwise rarely /write/ assembly (unopnionanted on at&t vs intel for gdb)
...
Org-babel export are wonky in latex... unsure if this is a wolfram-mode issue or not
#+begin_src elisp
...
*** Sage
Unsure if I need to specify the ~deferred~ dependency myself...
#+begin_src elisp
...
* TODO List
** DONE Magit: Place after evil config to set help menu correctly
** Mu4e: Get it to work with outlook (consistently)
Also make it so I don't have to log back in every day
** Evil: Add visual *​, evil-snipe, and maybe number incrementing
...
** Figure out twitter color emoji
** Latex org setup (very arduous)
*** org chameleon and engrave-faces
...
** DONE Fix zspell/ispell
** Dictionary mode
** TREESITTER
** DONE Org: Add lettered lists
** Ediff: Breaks persp-mode
** DONE Org babel: Don't export code results
