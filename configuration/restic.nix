...
let
  launchdTypes = import "${inputs.darwin.outPath}/modules/launchd/types.nix" { inherit config lib; };
in
{
        ...
        { name, ... }:
        {
          options = {
            passwordFile = lib.mkOption {
              ...
              '';
              example = "/etc/nix-darwin/restic-password";
            };
            ...
            timerConfig = lib.mkOption {
              type = lib.types.nullOr launchdTypes.StartCalendarInterval;
              default = [
                {
                  Day = 1;
                }
              ...
              description = ''
                When to run the backup. See {manpage}`launchd.plist(5)` for
            ...
            createWrapper = lib.mkOption {
              ...
              description = ''
                ...
                as the launchd service. This can be used to e.g. mount snapshots or perform other opterations, without
  ...
  config = {
    ...
    launchd.daemons = lib.mapAttrs' (
      ...
      let
        ...
        pruneCmd = lib.optionalString (builtins.length backup.pruneOpts > 0) ''
          ${resticCmd} unlock
          ${resticCmd} forget --prune ${lib.concatStringsSep " " backup.pruneOpts}
        '';
        checkCmd = lib.optionalString backup.runCheck ''
        ...
        preStart = ''
          ...
          ''}
        '';
        postStop = lib.optionalString (doBackup || backup.backupCleanupCommand != null) ''
      ...
      lib.nameValuePair "restic-backups-${name}" (
        {
          command = pkgs.writeShellScript "restic-backup-${name}" ''
            ${preStart}
            ${lib.optionalString doBackup
              "${resticCmd} backup ${
          ...
          environment =
            ...
            } // lib.optionalAttrs (backup.repository != null) {
          ...
          serviceConfig =
            {
              ...
              WorkingDirectory = "/run/restic-backups-${name}";
              StandardErrorPath = "/tmp/restic-backups-${name}.err.log";
              StandardOutPath = "/tmp/restic-backups-${name}.out.log";
            };
        }
    ...
    environment.systemPackages = lib.mapAttrsToList (
      ...
      pkgs.writeShellScriptBin "restic-${name}" ''
        ...
        # set same environment variables as the launchd service
        ${lib.pipe config.launchd.daemons."restic-backups-${name}".environment [
